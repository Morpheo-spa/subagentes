...
| Suite completa sobre PostgreSQL real | 278 tests, `TEST_DATABASE_URL` apuntando a Postgres 16. Misma cifra sobre SQLite con claves foráneas activas |
...
| Segunda auditoría de seguridad | `docs/SECURITY-AUDIT-2.md`: 13 hallazgos nuevos sobre los arreglos de la primera; 13 cerrados con test de regresión (`tests/test_security_audit_2.py`) |
...
- De `SECURITY-AUDIT-2.md`, todo cerrado; queda un residual de N-08: la dirección
  resuelta al guardar un backend no se pina, así que un rebinding DNS posterior no se
  detecta (sin redirecciones y con la lista de rangos internos, el daño es un oráculo).
