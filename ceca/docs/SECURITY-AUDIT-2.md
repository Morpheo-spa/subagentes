...
| N-07 | **Cerrado** | `_BoundedMultiPartParser`: cada parte con fichero se corta en `on_part_data` al superar `max_upload_mb` (`UPLOAD_FILE_TOO_LARGE`, 413, con el nombre), antes de volcarse a disco. Traefik: `maxRequestBodyBytes` bajado de 600 MB a 506 MB (justo sobre el techo de la API) e `inFlightReq: 32` por cliente en el router de la API; `/tmp` del contenedor `api` es un tmpfs acotado a 1 GB | `test_security_audit_2.py::test_a_file_part_over_the_limit_stops_the_stream_at_the_limit`, `test_infra_hardening.py::test_the_api_router_caps_requests_in_flight_and_spools_to_a_bounded_tmpfs` |
| N-08 | **Cerrado** | La resolución DNS ocurre una sola vez, al guardar el backend, en un hilo (`validate_config_offloaded` → `asyncio.to_thread`). Los adaptadores S3 y FTP revalidan en cada petición con `resolve=False`: esquema, puerto y dirección literal, sin red. Residual: no se pina la dirección resuelta, así que un rebinding DNS posterior al guardado no se detecta (mitigado por N-04: sin redirecciones) | `::test_the_adapters_check_a_name_without_resolving_it`, `::test_saving_a_backend_resolves_in_a_worker_thread`, `::test_the_storage_router_never_calls_the_blocking_validator` |
...
| N-11 | **Cerrado** | `X-Request-ID` del cliente solo se acepta si cumple `^[A-Za-z0-9._-]{1,64}$`; si no, se sustituye por uno propio | `::test_an_unacceptable_request_id_is_replaced`, `::test_the_response_echoes_only_a_bounded_request_id` |
